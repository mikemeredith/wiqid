...
# test_that code for cjs series of functions
...
test_that("cjs0 gives right answers",  {
  ...
  res <- cjs0(dippers)
  expect_that(class(res), equals("matrix"))
  expect_that(colnames(res), equals(c("est", "lowCI", "uppCI")))
  expect_that(rownames(res), equals(c("phiHat", "pHat")))
  expect_that(round(as.vector(t(res)), 4), 
      equals(c(0.5602, 0.5105, 0.6088, 0.9026, 0.8305, 0.9460))) # MARK output
  expect_that(round(attr(res, "AIC"), 4), equals(670.8377)) # MARK output

  res <- cjs0(dippers, 0.85)
  expect_that(round(as.vector(t(res)), 4), 
      equals(c(0.5602, 0.5238, 0.5960, 0.9026, 0.8530, 0.9367)))
}  )
# .........................................................................

test_that("cjsTime gives right answers",  {
  # Data set (Lereton et al dippers)
  dippers <- matrix(c(
    11,2,0,0,0,0,9,
    0,24,1,0,0,0,35,
  ...
  res <- cjsTime(dippers)  # default is also a phi(.) p(.) model
  expect_that(class(res), equals("list"))
  expect_that(names(res), equals(c("beta", "real", "AIC")))
  ...
  expect_that(round(as.vector(t(res$real[6:7,])), 4), 
      equals(c(0.5602, 0.5105, 0.6088, 0.9026, 0.8305, 0.9460))) # MARK output
  expect_that(round(res$AIC, 4), equals(670.8377)) # MARK output

  res <- cjsTime(dippers, phi = ~time) 
  ...
  expect_that(round(res$AIC, 4), equals(673.7301)) # MARK output

  dd <- data.frame(flood = c(0, 1, 1, 0, 0, 0))
  res <- cjsTime(dippers, phi = ~flood, data=dd)
  ...
  expect_that(round(res$AIC, 4), equals(666.1028)) # MARK output

  res <- cjsTime(dippers, phi = ~flood, data=dd, ci=0.85)
  expect_that(round(as.vector(t(res$real[c(1,2,7),])), 4), 
