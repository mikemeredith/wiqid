...
test_that("BoccSS gives same answers",  {
  # Use the weta data
  data(weta)
  DH <- weta[, 1:5]
  weta.covs <- weta[, 6:11]
  ...
  set.seed(123)
  expect_output({Bout <- BoccSS(DH, n.iter=1100)},
  ...
  expect_that(attr(Bout, "header"), equals("Model fitted in R with a Gibbs sampler"))
  expect_that(attr(Bout, "n.chains"), equals(3))
  expect_equivalent(round(attr(Bout, "n.eff")), c(155, 276))
  expect_equivalent(round(attr(Bout, "Rhat"), 3), c(1.006, 1.002))
  expect_equal(as.character(attr(Bout, "call")), c("BoccSS", "DH", "1100"))

  expect_equivalent(round(colMeans(Bout), 4), c(0.4041, -0.4213))
  expect_equivalent(round(c(hdi(Bout)), 4), c(-0.1174, 1.0262, -0.7077, -0.1118))

  set.seed(234)
  expect_output({Bout <- BoccSS(DH, model=list(psi~Browsed-1, p~.Time), data=weta,
    priors=list(sigmaPsi=c(1,1)), n.chains=2, n.iter=1100)}, " ")
  expect_that(class(Bout), equals(c("Bwiqid", "data.frame")))
  expect_that(dim(Bout), equals(c(2000, 4)))
  expect_that(attr(Bout, "n.chains"), equals(2))
  expect_equivalent(round(attr(Bout, "n.eff")), c(354, 99, 283, 958))
  expect_equivalent(round(attr(Bout, "Rhat"), 3), c(1.022, 1.079, 1.027, 1.000))
  expect_equal(as.character(attr(Bout, "call")), 
    c( "BoccSS", "DH", "list(psi ~ Browsed - 1, p ~ .Time)",
    "weta", "list(sigmaPsi = c(1, 1))", "2", "1100"))

  expect_equivalent(round(colMeans(Bout), 4), c(0.0215, 0.8812, -0.4329, 0.3375))
  expect_equivalent(round(c(hdi(Bout)), 4),
    c(-0.5049, 0.5942, -0.0049, 1.8744, -0.7175, -0.1657, -0.0554, 0.7519))
})